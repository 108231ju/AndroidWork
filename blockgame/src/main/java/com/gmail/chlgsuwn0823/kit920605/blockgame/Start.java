package com.gmail.chlgsuwn0823.kit920605.blockgame;
...
public class Start extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        // 버튼을 눌렸을떄 호출되는 콜백
        // 블럭과 같은 색깔의 눌렀는지 판별, 같은 블럭이면 이미지 블럭 한칸씩 내려오기
        // 맨 위에는 새로운 블럭 생성
        ...
        switch (imageView.getId()){
            ...
            case R.id.ivGreen:
                ...
                break;  // 초록버튼 클릭시
            case R.id.ivBlue:
                if("2".equals(iv[7].getTag().toString())) isOK = true;  // 파란블럭의 tag 값 "2"
                break;  // 파란버튼 클릭시

        }
        if (isOK){  // 버튼 색깔을 맞추었다면!

            // 위의 7개 블럭을 한칸 아래로 이동 iv[1] -> iv[i + 1]
            for(int i = iv.length -2; i >= 0; i--){
                ...
                iv[i + 1].setTag(num + "");     // 아래쪽 블럭 tag 값 업데이트
            }// end for
